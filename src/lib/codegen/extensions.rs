...
impl<'a> ExtGen<'a> {
    fn extendee_rust_name(&self) -> String {
        type_name_to_rust_relative(
            self.field.get_extendee(), self.file, true, self.root_scope)
    }
    ...
    fn return_type_gen(&self) -> ProtobufTypeGen {
        if self.field.has_type_name() {
            let rust_name_relative = type_name_to_rust_relative(self.field.get_type_name(), self.file, true, self.root_scope);
            match self.field.get_field_type() {
                FieldDescriptorProto_Type::TYPE_MESSAGE =>
                    ProtobufTypeGen::Message(rust_name_relative),
                FieldDescriptorProto_Type::TYPE_ENUM =>
                    ProtobufTypeGen::Enum(rust_name_relative),
    ...
    fn write(&self, w: &mut CodeWriter) {
        let suffix = if self.repeated() { "Repeated" } else { "Optional" };
        let field_type = format!("::protobuf::ext::ExtField{}", suffix);
        w.pub_const(
            self.field.get_name(), // TODO: escape
            &format!("{}<{}, {}>",
                field_type, self.extendee_rust_name(), self.return_type_gen().rust_type()),
            &format!("{} {{ field_number: {}, phantom: ::std::marker::PhantomData }}",
                field_type, self.field.get_number()));
    }
}
