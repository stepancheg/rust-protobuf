...
use super::test_fmt_text_format_0_pb::*;
...
fn test_repeated_one() {
    ...
    t(
        "fixed32_repeated: 99",
        |m| m.fixed32_repeated.push(99),
    );
    t(
        "fixed64_repeated: 99",
        |m| m.fixed64_repeated.push(99),
    );
    t("sfixed32_repeated: 99", |m| {
        m.sfixed32_repeated.push(99)
    });
    t("sfixed64_repeated: 99", |m| {
        m.sfixed64_repeated.push(99)
    });
    t(
        "bool_repeated: false",
        |m| m.bool_repeated.push(false),
    );
...
fn test_pretty() {
    ...
    tm.set_value(23);
    let mut m = TestTypes::new();
    m.test_message_singular.set_message(tm);
    m.set_string_singular("abc".to_string());
    m.string_repeated.push("def".to_string());
    m.string_repeated.push("ghi".to_string());
    assert_eq!(
