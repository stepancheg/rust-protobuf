...
fn file_descriptor_dynamic() -> FileDescriptor {
    FileDescriptor::new_dynamic(test_is_initialized_pb::file_descriptor_proto().clone(), Vec::new())
}
...
fn is_initialized_dynamic() {
    ...
    let message_descriptor = file_descriptor.message_by_package_relative_name("TestIsInitialized").unwrap();
