    ...
    if field.field.get_field_type() == FieldDescriptorProto_Type::TYPE_GROUP {
        ...
        let elem = match field.field.get_field_type() {
            FieldDescriptorProto_Type::TYPE_STRING if carllerche_for_string =>
                FieldElem::Primitive(FieldDescriptorProto_Type::TYPE_STRING, PrimitiveTypeVariant::Carllerche),
            FieldDescriptorProto_Type::TYPE_BYTES if carllerche_for_bytes =>
                FieldElem::Primitive(FieldDescriptorProto_Type::TYPE_BYTES, PrimitiveTypeVariant::Carllerche),
...
impl<'a> FieldGen<'a> {
        ...
        match field_type_size(self.proto_type) {
            ...
            None => {
                match self.proto_type {
                    ...
                    FieldDescriptorProto_Type::TYPE_BYTES => {
                        format!("::protobuf::rt::bytes_size({}, &{})",
                            self.proto_field.number(), var)
                    },
                    FieldDescriptorProto_Type::TYPE_STRING => {
                        format!("::protobuf::rt::string_size({}, &{})",
                            self.proto_field.number(), var)
                    },
                    FieldDescriptorProto_Type::TYPE_ENUM => {
                        ...
                        format!("::protobuf::rt::enum_size({}, {})",
                            self.proto_field.number(), var_type.into_target(&param_type, var))
                    },
                    _ => {
                        let param_type = match var_type {
                            &RustType::Ref(ref t) => (**t).clone(),
        ...
        match self.proto_type {
            FieldDescriptorProto_Type::TYPE_MESSAGE => {
                w.write_line(&format!("{}.write_tag({}, ::protobuf::wire_format::{:?})?;",
                    os, self.proto_field.number(), wire_format::WireTypeLengthDelimited));
    ...
    fn self_field_vec_packed_varint_size(&self) -> String {
        ...
        format!("::protobuf::rt::{}({}, &{})",
            fn_name, self.proto_field.number(), self.self_field())
    }
    ...
    pub fn write_element_size(&self, w: &mut CodeWriter, item_var: &str, item_var_type: &RustType, sum_var: &str) {
        assert!(!self.is_repeated_packed());
