...
fn protoc_descriptor_set(includes: &[PathBuf], inputs: &[PathBuf]) -> FileDescriptorSet {
    ...
    let includes: Vec<&str> = includes.iter().map(|p| p.to_str().unwrap()).collect();
    let inputs: Vec<&str> = inputs.iter().map(|p| p.to_str().unwrap()).collect();
...
fn pure_descriptor_set(includes: &[PathBuf], inputs: &[PathBuf]) -> FileDescriptorSet {
    let includes: Vec<&Path> = includes.iter().map(|p| p.as_path()).collect();
    let inputs: Vec<&Path> = inputs.iter().map(|p| p.as_path()).collect();
    let mut codegen = protobuf_codegen_pure::parse_and_typecheck(&includes, &inputs).unwrap();
    let relative_paths: HashSet<_> = codegen.relative_paths.iter().map(|s| s.as_str()).collect();
    codegen
        .file_descriptors
        .retain(|fd| relative_paths.contains(fd.get_name()));
    let mut fds = FileDescriptorSet::new();
    fds.file = codegen.file_descriptors.into();
    fds
}
